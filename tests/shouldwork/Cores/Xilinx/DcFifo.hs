...
fifoSampler clk rst ena stalls inps =
  mealyB clk rst ena go False (stalls, inps)
 where
  go ::
    ReadLastCycle ->
    (Stall, (ResetBusy, Empty, DataCount depth, BitVector n)) ->
    (ReadLastCycle, Maybe (BitVector n))
  go _             (_,     (True,      _,     _,         _       )) = (False, Nothing)
  go readLastCycle (stall, (_resetBusy, fifoEmpty, _dataCount, readData)) = (readNow, maybeData)
   where
...
testBench = done
 where
  ...
  maybeReadData = fifoSampler rClk rRst rEna rLfsr (bundle (readReset, isEmpty, readCount, fifoData))
...
fifoVerifier clk rst ena = mealy clk rst ena go 0
 where
  ...
  go expected (Just actual) = (succ expected, expected /= actual)
